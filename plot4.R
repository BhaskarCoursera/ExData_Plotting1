# read file
felcons <- read.csv("household_power_consumption.txt",header = TRUE, sep = ";",na.strings = "?",stringsAsFactors = FALSE)

# subset the rows within 2 days 2/1/2007 or 2/2/2007
elconsub <- felcons[felcons$Date %in% c('1/2/2007', '2/2/2007'),]
...
par(mfrow = c(2,2))
with(elconsdf1, {
  plot(elconsdf1$Date_Time,elconsdf1$Global_active_power,ylab= "Global Active Power", type= "l",xlab = "")
  plot(elconsdf1$Date_Time,elconsdf1$Voltage,ylab= "Voltage", type= "l",xlab = "datetime")
  ...
  plot(elconsdf1$Date_Time,elconsdf1$Global_reactive_power,ylab= "Global_reactive_power", type= "l",xlab = "datetime")
})
dev.off()